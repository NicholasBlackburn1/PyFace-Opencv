...
while True:
    ...
    for face in faces:
        ...
        for n in range(0, 68):
            ...
            left_eye_center_top = midpoint(landmarks.part(37), landmarks.part(37))
            left_eye_center_bottom = midpoint(landmarks.part(40), landmarks.part(40))
            ...
            top_left_eye = (int(left_eye_left_point[0] -eyeL_width / 2),
                              int(left_eye_left_point[1] - eyeL_height / 2))
            
            bottom_right = (int(left_eye_right_point[0] + eyeL_width / 2),
                int(left_eye_right_point[1] + eyeL_height / 2))
            
            dim =(int(nose_width),int(nose_height))
            eyeLdim =(int(eyeL_width),int(eyeL_height))
            ...
            cv2.putText(frame,'eyeLEFT pos new'+str(eyeLdim), 
            ...
            eyeL_pig = cv2.resize(eye_image, eyeLdim)
            #eyeL_gray = cv2.cvtColor(eyeL_pig, cv2.COLOR_BGR2GRAY)
            #_,  eyeLmask = cv2.threshold(eyeLmask, 25, 255, cv2.THRESH_BINARY_INV)   
            # eyeL_area_no_nose = cv2.bitwise_and(eyeL_area, eyeL_area, mask=eyeLmask)
           #final_eye = cv2.add(eyeL_area_no_nose, eyeL_pig)
            #frame[int(top_left[1]): int(top_left[1]) + int(eyeL_height),
            #int(top_left_eye[0]): int(top_left_eye[0]) + int(eyeL_width)] = final_eye
            plt.title("face Graphic")
            plt.plot(dim,eyeLdim)    
            plt.show()        
            #cv2.imshow("Nose area", frame)
